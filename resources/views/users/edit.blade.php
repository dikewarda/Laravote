                    ...
                    <div class="card-body">
                        ...
                        <form enctype="multipart/form-data" action="{{route('users.update',['id'=>$user->id])}}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="address" cols="5" rows="5" class="form-control {{ $errors->first('address') ? "is-invalid" : ""}}">{{$user->address}}</textarea>
                                <p class="text-danger">{{ $errors->first('address') }}</p>
                            </div>
                            <div class="form-group">
                                <label for="">No Telp</label>
                                <input type="text" name="phone" id="phone" value="{{$user->phone}}" class="form-control {{ $errors->first('phone')  ? "is-invalid" : ""}}">
